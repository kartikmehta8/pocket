...
| POST   | `/v1/agents`                                 | `{ name, description?, metadata? }`                               | `{ agent, wallet, seed }` — `seed` is `{ amount \| null, txHash \| null }`                                 |
| GET    | `/v1/agents?limit&cursor&status&q`           |                                                                   | `{ agents: AgentSummary[], nextCursor }` — unpaged without `limit`; `q` searches name, description, wallet |
| GET    | `/v1/agents/:id`                             |                                                                   | `{ agent, policy \| null, taskBudgets, balance \| null, accountId \| null, accountHollow \| null }`        |
| PATCH  | `/v1/agents/:id`                             | `{ name?, status?, description?, metadata? }`                     | `{ agent }`                                                                                                |
| DELETE | `/v1/agents/:id?force`                       |                                                                   | `204` — refused with `CONFLICT` while the wallet holds funds, unless `force=true`                          |
| POST   | `/v1/agents/:id/transfer`                    | `{ toAgentId, asset? }`                                           | `{ transfer: { amount, asset, txHash, explorerUrl \| null } }` — moves the whole balance                   |
| POST   | `/v1/agents/:id/wallet/activate`             |                                                                   | `{ accountId, activated, txHash \| null, alreadyActive, confirmed, seed }`                                 |
...
| GET    | `/v1/payments?agentId&status&limit&cursor`   |                                                                   | `{ payments: Payment[], nextCursor }`                                                                      |
...
| GET    | `/v1/audit?limit=50&cursor&action&actorType` |                                                                   | `{ events: AuditEvent[], nextCursor }` — `action` is a family prefix such as `payment`                     |
