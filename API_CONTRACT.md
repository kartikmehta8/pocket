...
| ------ | ------------------------------------------ | ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| POST   | `/v1/auth/session`                         | `{ orgName? }` + identity token as bearer                         | `{ org, user, provisioned, apiKey }` — `201` and a one-time key when it created the tenant                |
| POST   | `/v1/orgs`                                 | `{ name }`                                                        | `{ org, apiKey }` — key shown once                                                                        |
...
| DELETE | `/v1/api-keys/:id`                         |                                                                   | `{ key }` — session only; the last live key cannot be revoked                                             |
...
| GET    | `/v1/agents/:id`                           |                                                                   | `{ agent: AgentSummary, policy: Policy \| null, taskBudgets: TaskBudget[], balance: Balance \| null }`    |
| PATCH  | `/v1/agents/:id`                           | `{ status?, description?, metadata? }`                            | `{ agent }`                                                                                               |
| PUT    | `/v1/agents/:id/budget`                    | `{ asset, dailyLimit, perTransactionLimit }`                      | `{ budget }`                                                                                              |
| PUT    | `/v1/agents/:id/policy`                    | Policy without `approvalThreshold: null`                          | `{ policy }`                                                                                              |
