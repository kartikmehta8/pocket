...
`http://localhost:8081/` describes the running server — the transport, the
credential it wants, and every tool with whether that tool can spend.

## Connect an agent

```bash
hermes mcp add pocket --transport http --url http://localhost:8081/mcp \
  --header "Authorization: Bearer pocket_sk_..."
```

```bash
claude mcp add --transport http pocket http://localhost:8081/mcp \
  --header "Authorization: Bearer pocket_sk_..."
```

Any other MCP client takes the same endpoint and header.
...
Each request gets its own server, transport and API client, so nothing — state
or credential — crosses between callers.
